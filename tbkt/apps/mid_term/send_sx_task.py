...
from apps.mid_term.common import get_text_content, get_ticket
...
def send_sms_method_with_ids(request, status, unit_ids, sendscope, content, open_content, sendpwd):
    if status == 1:
...
def p_give_web(request):
    ...
    open_content = get_text_content(content, sendscope, grade_id, tea_name, end_time, subject_id=21)
    if not type:
    ...
    with db.tbkt_shuxue as dt:
        ...
        call(send_sms_method_with_ids,request, status, unit_ids, sendscope, content, open_content, sendpwd)
